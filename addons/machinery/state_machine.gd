...
func _ready() -> void:
  ...
  change_state(null, initial_state)
...
func change_state(from: State, to: State) -> void:
  ...
  if debug_change_state:
    print("Changed from %s to %s" % [from.name if from else "null", to.name])


## Changes the StateMachine's state by name. If the state does not exist,
## noting happens.
func change_state_by_name(from: State, to_name: String) -> void:
  var to_state := state_by_name(to_name)

  if to_state == null:
    return

  change_state(from, to_state)
